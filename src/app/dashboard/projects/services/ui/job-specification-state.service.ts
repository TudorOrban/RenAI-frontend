...
export class JobSpecificationStateService {
    private initialSpecification: JobSpecification = {
        appSpecification: { appName: "", functionalSpecifications: [] },
        developmentSpecification: {}
    };
    private initialState: SpecificationManagerState = {
        editingState: {
            editedSpecification: this.initialSpecification,
            isEditModeOn: false
        },
        autoGenerationState: {
            autoGeneratedSpecification: this.initialSpecification,
            isAutoGenerationModeOn: false
        },
        renderType: SpecificationRenderType.JSON
    }

    private stateSubject = new BehaviorSubject<SpecificationManagerState>(this.initialState);
    state$ = this.stateSubject.asObservable();

    initializeService(jobSpecification?: JobSpecification): void {
        if (!jobSpecification) return;
        
        this.setEditedSpecification(jobSpecification);
        this.setAutoGeneratedSpecification(jobSpecification);
    }
